...
//! `bind_identity`, and `build_rejection` are fuzzed here too since they
//! parse the same untrusted frame on other paths (StartTLS negotiation,
//! identity tracking, and rejecting a blocked request) and share the same
//! risk (a malformed/adversarial frame panicking instead of returning
//! `Err`). None of these should ever panic — an error is a fine outcome, a
//! panic isn't.
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = CONNECTOR.bind_identity(data);
